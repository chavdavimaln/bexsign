...
import { generateEmployeeSignatureId, signatureIdLines } from '../utils/documentId';
...
export default function SignatureStamp({
  ...
  // Determine unique signature ID: on a document it carries the signer's own initials and the document ID
  const fromDocument = !signId && typeof docId === 'string' && (docId.startsWith('BEX-SIGN') || docId.startsWith('BEX-DOC'));
  const effectiveSignId = signId
    ? signId
    : (fromDocument ? signatureIdLines(docId, signerName).join('-') : generateEmployeeSignatureId(employeeId, signerName));

  // Display ID cleanly (if very long, split into 2 neat lines)
  let idLine1 = effectiveSignId;
  let idLine2 = '';
  if (fromDocument) {
    [idLine1, idLine2] = signatureIdLines(docId, signerName);
  } else if (effectiveSignId.length > 26) {
